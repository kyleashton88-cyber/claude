...
| **Lesson videos (incl. Mastery Starters)** | **121** | `video-scripts/lessons/*.json` + `video-scripts/gold/*.json` → `video/lesson-*.mp4` | Scripts ready (Lesson 3.2 gold standard); 2 rendered |
| Strategy library series (8.3) | 7 (one per level) | to be scripted (see prompt) | To do |

Estimated total runtime of lesson videos from the baseline scripts: **5.5 hours**.
...
| 2.2 | AMM Mathematics (x · y = k) | 4 | 0.6 min | `video-scripts/lessons/lesson-02-2.json` |  |
