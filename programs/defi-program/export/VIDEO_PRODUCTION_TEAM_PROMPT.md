...
5. **Best visual for every idea — in this order of preference:**
   ...
   - `flow3d`/`chart3d` are **experimental, opt-in** WebGL/Three.js alternatives to
     `flow`/`chart` (bars and donut only, no `line` yet) — same JSON, just change `"type"` —
     rendering floating holographic nodes or glowing particle-stream bars/donut instead of
     flat cards and SVG. Read the `webgl-motion-graphics` skill before using either: it has
     the measured render-time/file-size cost, the node-count and portrait caveats, the
     shape/camera variety system (auto-picked per scene so lessons don't look copy-pasted),
     and eight real reference screenshots with the bugs each one caught. Use **sparingly — at
     most one or two scenes per lesson** for a mechanism worth visually anchoring the lesson
     around, not a wholesale replacement for the flat scenes; re-check final lesson file size
     against the ~95MB cap if used on more than that. A shader-based ambient background does
     not exist yet; don't invent it.
