...
function visual(s) {
  return { strike: `"${s.big}" struck out → "${s.after}"`, statement: (s.lines || []).join(' / ') + (s.sub ? ` — ${s.sub}` : ''), bullets: `${s.title}: ${(s.items || []).join(' · ')}`,
    ...
    compare: `${s.title}: ${s.left.label} vs ${s.right.label}`, steps: `${s.title}: ${(s.steps || []).join(' → ')}${s.result ? ` ⇒ ${s.result}` : ''}`, quiz: `Quiz: ${s.q} → ${s.a}`,
    flow: `Flow: ${(s.nodes || []).map(n => n.label).join(' → ')}`, cutaway: `Screen cutaway: ${(s.shots || []).map(x => x.caption || x.src).join(' → ')}` }[s.type];
}
