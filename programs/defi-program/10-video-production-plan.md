...
| 2.0 | Mastery Starter | 29 | 9.9 min | `video-scripts/gold/lesson-02-0.json` (gold: swap/pool mechanism flow, price-impact and slippage-vs-impact compares, a real LP fee-APR stat, an animated LP-vs-holding impermanent-loss line chart, DEX-vs-aggregator and single/multi-hop routing compares, animated ladder) | ✓ |
| 2.1 | DEXs, aggregators & routing | 28 | 9.6 min | `video-scripts/gold/lesson-02-1.json` (gold: AMM-pool vs order-book flows, aggregator/routing flows with a due-diligence steps scene, a real DEX-volume stat, the quote-minus-everything formula flow with a pool-depth compare, and the source material's own 20 ETH / 0.5 ETH worked-example numbers) | ✓ |
| 2.2 | AMM Mathematics (x · y = k) | 28 | 10.1 min | `video-scripts/gold/lesson-02-2.json` (gold: the constant-product formula as a two-directional flow, the x·y=k curve image with a "not a fee" statement, an arbitrage-rebalancing flow, and the source material's own 100 ETH/300,000 USDC worked example extended to a 3-point 1/10/50 ETH price-impact line chart) | ✓ |
| 2.3 | Providing liquidity | 26 | 9.7 min | `video-scripts/gold/lesson-02-3.json` (gold: LP-deposit/withdraw flows, a full-range-vs-concentrated compare, the fee-APR formula as a flow with a full-range simplification, a second $5M-pool stat pair, a fee-tier-vs-volume compare, and the source material's own $10M TVL / $2M volume worked example with a TVL/volume sensitivity compare) | ✓ |
| 2.4 | Impermanent loss & true LP P&L | 27 | 9.9 min | `video-scripts/gold/lesson-02-4.json` (gold: the IL formula as a flow, a symmetric-divergence compare and line chart, the full LP P&L formula as a flow, a weekly-tracking steps scene, a gas-cost-at-scale compare, and the source material's own $6,000 worked example judged on the $58 vs the $1,058) | ✓ |
| 2.5 | MEV and protecting your trades | 28 | 9.8 min | `video-scripts/gold/lesson-02-5.json` (gold: arbitrage-vs-sandwich MEV compare, a real MEV-extraction stat, the sandwich-mechanism image and flow, a check-a-past-trade steps scene, private-routing flow, a turning-it-on steps scene, and the source material's own $20,000 worked example extended to a 4-point exposure-vs-slippage line chart) | ✓ |
| 2.6 | Advanced execution: limit, TWAP and intent-based orders | 27 | 9.5 min | `video-scripts/gold/lesson-02-6.json` (gold: limit/TWAP/intent-order flows with fit and trade-off compares, a TWAP-setup and get-all-three-quotes steps scene, a real intent/RFQ-adoption stat, and the source material's own $50,000 worked example comparing a single swap, a TWAP, and an intent order) | ✓ |
| 2.7 | Advanced AMM design and LVR | 28 | 10.7 min | `video-scripts/gold/lesson-02-7.json` (gold: AMM-design compares/flows including a concentrated-liquidity tick-range flow, the LVR formula as a flow with the lvr.png curve and an arbitrage-vs-LVR reconciliation compare, the source material's own 80%-vs-110%-volatility worked example with a calculator-usage steps scene, and hooks/contract-risk) | ✓ |
| 2.8 | The MEV supply chain | 27 | 10.1 min | `video-scripts/gold/lesson-02-8.json` (gold: the full transaction path as two flows plus the mev-supply-chain.png image, a real MEV-extraction stat, rebate mechanism/checking-your-RPC/sizing compares and steps, L2-sequencer compare plus a decentralization-roadmap flow, and the source material's own $100,000 worked example. Closes Module 2) | ✓ |
...
| 3.4 | Borrowing strategies & looping | 27 | 10.0 min | `video-scripts/gold/lesson-03-4.json` (gold: sell-vs-borrow and correlated-vs-volatile compares, a loop-mechanism flow, the leverage formula plus a 5-point loops-vs-leverage line chart, the loop-spread.png image, and the source material's own 2.53x/5.03%-net-APY worked example with a break-even-rate compare and an ongoing-monitoring steps scene) | ✓ |
| 3.5 | Perpetual futures and margin on-chain | 28 | 9.3 min | `video-scripts/gold/lesson-03-5.json` (gold: perp-anatomy.png walk-through, an isolated-vs-cross-margin compare, a mark/index/funding flow with a why-mark-price statement, the liquidation formula as a flow, the source's own 5x/20x worked example as steps+compare, a 6-point room-to-be-wrong-vs-leverage line chart with a doubling-halves-your-room stats scene, a defi_calc.py perp steps scene, and a short-vs-long holding period funding compare) | ✓ |
